var g_Backup_FileName = "avr2013_backup.dat";
var g_RecoveryStatus = false;
 ...
 var g_modelNameSettingsOnly = g_modelNameLow;
 var g_modelName6xx = [
 "RX-V675",
 "HTR-6066",
 "RX-A730",
 ...
 var g_modelNameExtraHigh = [
 "CX-A5000"
 ];
 var g_Zone2_HdmiAvail = [
 "RX-V1075",
 "RX-A1030",
 "RX-V2075",
 "RX-A2030",
 ...
 if( g_Info.blnModelSettingsOnly() )
 {
 ...
 availPage[g_SettingPage_iPhone] = 0;
 GetEleId("renameinfo_NetworkZoneName").style.display = "none";
 ...
 {
 GetEleId("renameinfo_NetworkNameOnly").style.display = "none";
 }
 ...
 var nameArray = [];
 if( g_Info.blnModelSettingsOnly()==false ){
 ...
 switch(l_zone){
 case 0:
 if(( g_Info.blnModelTypeSearch(g_modelNameHigh) == true ) || ( g_Info.blnModelTypeSearch(g_modelNameExtraHigh) == true )){
 iidx = gInput2IdxForMainOfHigh[g_inputName[i]];
 }else{
 iidx = gInput2IdxForMainOfLow[g_inputName[i]];
 if( g_Info.blnModelTypeSearch(g_modelName8xx) == true ){
 }else{
 ...
 break;
 case 1:
 if( g_Info.blnModelTypeSearch(g_Zone2_HdmiAvail) == true ){
 ...
 if( g_inputName[i] == g_NapsterName ) {
 if( g_Info.blnExistNapster() == false ) { continue; }
 }
 if( g_inputName[i] == g_SiriusXMName ) {
 if( g_Info.blnExistSiriusXM() == false ) { continue; }
 }
 ...
 if( g_inputName[i] == g_SpotifyName ) {
 if( g_Info.blnExistSpotify() == false ) { continue; }
 }
 iname = g_inputName[i];
 ...
 if(( g_Info.blnModelTypeSearch(g_modelNameHigh) == false ) && ( g_Info.blnModelTypeSearch(g_modelNameExtraHigh) == false )){
...
var g_fadeInFrameSrc = null;
 ...
 var blnRet = true;
 if( this.blnModelTypeSearch(g_modelName6xx) == true )
 {
 blnRet = false;
 }
 return blnRet;
 ...
 var Num=0;
 if(this.blnModelSettingsOnly()==false)
 {
 Num=1;
 ...
 if( this.blnModelTypeSearch(g_modelName6xx) == true ) {
 blnRet = false;
 }else if(( this.blnModelTypeSearch(g_modelNameHigh) == true ) || ( this.blnModelTypeSearch(g_modelNameExtraHigh) == true )){
 }else{
 ...
 var blnType = 0;
 if( g_Info.blnModelTypeSearch(g_modelName20xx) == true )
 {
 blnType = 1;
 }
 ...
 else if( g_Info.blnModelTypeSearch(g_modelNameExtraHigh) == true )
 {
 blnType = 3;
 }
 return blnType;
 };
 this.blnModelSettingsOnly = function() {
 if( g_Info.blnModelTypeSearch(g_modelNameSettingsOnly) == true )
 {
 return true;
 }else{
 return false;
 ...
 this.m_existNapster = false;
 this.blnExistNapster = function() {
 return this.m_existNapster;
 };
 this.m_existSiriusXM = false;
 this.blnExistSiriusXM = function() {
 return this.m_existSiriusXM;
 };
 this.m_existPandora = false;
 this.blnExistPandora = function() {
 return this.m_existPandora;
 };
 this.m_existSpotify = false;
 this.blnExistSpotify = function() {
 return this.m_existSpotify;
 };
 this.m_macAddr = ["","","","","","","","","",""];
 this.m_ipDHCP = "On";
 this.m_ipIP_Address = "";
...
function unescapeForXML(a) {
 ...
 if( surType == 0 ){
 if( checkSurroundOption(name,gPrgDifName_6xx_10xx) ){
 continue;
 }
 }else if((surType == 1) || (surType == 2)){
 if( checkSurroundOption(name,gPrgDifName_20xx_30xx) ){
 continue;
 }
 }else{
 if( checkSurroundOption(name,gPrgDifName_5000) ){
 continue;
 }
 }
